...
describe ThirdSocialMediaController do
  ...
  let(:current_user_json) do
    %Q({"_id":"56608bd7edd9adc72c1b48ee","provider":"local","name":"Pedro","email":"pedrohrs08@gmail.com","password":"081289","__v":0,"role":"user"})
  end
  ...
  let(:friends_stubbed_response) do
  ...
  it "should export my data to json format when I enter my credentials" do
    ...
    allow(RestClient::Request).to receive(:execute).with(hash_including(url:/friendships\/me/))
                                      .and_return(friends_stubbed_response)
    allow(RestClient::Request).to receive(:execute).with(hash_including(url:/users\/me/))
                                      .and_return(user_stubbed_response)

    get :export, user: "my_user@me", password: "my_password"
    json_exported = JSON.parse(response.body)
    expect(json_exported['user']['name']).to eq "Pedro"
  end
end